...
def test_load_key_from_qr_code(mocker):
    ...
    cases = [
        (FORMAT_NONE, 'olympic term tissue route sense program under choose bean emerge velvet absurd', 'olympic term tissue route sense program under choose bean emerge velvet absurd'),
        (FORMAT_NONE, 'brush badge sing still venue panther kitchen please help panel bundle excess sign couch stove increase human once effort candy goat top tiny major', 'brush badge sing still venue panther kitchen please help panel bundle excess sign couch stove increase human once effort candy goat top tiny major'),
        (FORMAT_NONE, '123417871814150815661375189403220156058119360008', 'olympic term tissue route sense program under choose bean emerge velvet absurd'),
        (FORMAT_NONE, '023301391610171019391278098413310856127602420628160203911717091708861236056502660800183118111075', 'brush badge sing still venue panther kitchen please help panel bundle excess sign couch stove increase human once effort candy goat top tiny major'),
        (FORMAT_UR, UR('crypto-bip39', bytearray(binascii.unhexlify('A2018C66736869656C646567726F75706565726F6465656177616B65646C6F636B6773617573616765646361736865676C6172656477617665646372657765666C616D6565676C6F76650262656E'))), 'shield group erode awake lock sausage cash glare wave crew flame glove')
    ]
    for case in cases:
        ctx = mock.MagicMock(
            input=mock.MagicMock(wait_for_button=mock.MagicMock(
        ...
        login = Login(ctx)
        mocker.patch.object(login, 'capture_qr_code', mock.MagicMock(return_value=(case[1], case[0])))
    ...
    cases = [
        (
            ...
            (
                ...
                # I
                [BUTTON_PAGE for _ in range(7)] + [BUTTON_ENTER] +
                # Go + Confirm
                [BUTTON_ENTER, BUTTON_ENTER]
            ) * 11 + 
            (
                # N
                [BUTTON_PAGE for _ in range(13)] + [BUTTON_ENTER] +
                # O
                [BUTTON_PAGE, BUTTON_ENTER] +
                # R
                [BUTTON_PAGE for _ in range(3)] + [BUTTON_ENTER] +
                # T
                [BUTTON_PAGE for _ in range(2)] + [BUTTON_ENTER] +
                # Go
                [BUTTON_ENTER]
            ) +
    ...
    cases = [
        (
            ...
            (
                ...
                # Go + Confirm
                [BUTTON_PAGE for _ in range(9)] + [BUTTON_ENTER, BUTTON_ENTER]
            ) * 11 + 
            (
                # 1
                [BUTTON_PAGE, BUTTON_ENTER] +
                ...
                [BUTTON_PAGE, BUTTON_ENTER] +
                # 0
                [BUTTON_PAGE for _ in range(10)] + [BUTTON_ENTER] +
                # 3
                [BUTTON_PAGE, BUTTON_PAGE, BUTTON_PAGE, BUTTON_ENTER] +
                # Go
                [BUTTON_PAGE for _ in range(8)] + [BUTTON_ENTER]
            ) +
            # Done?, 12 word confirm, Continue?, Single-key
            [BUTTON_ENTER, BUTTON_ENTER, BUTTON_ENTER, BUTTON_ENTER],
            'ability ability ability ability ability ability ability ability ability ability ability north'
    ...
    cases = [
        (
            ...
            (
                ...
                [BUTTON_PAGE, BUTTON_ENTER] +
                # Go + Confirm
                [BUTTON_PAGE for _ in range(2)] + [BUTTON_ENTER, BUTTON_ENTER]
            ) * 11 + 
            (
                # 100 10 11 00 10
                # 1
                [BUTTON_PAGE, BUTTON_ENTER] +
                # 00
                [BUTTON_PAGE for _ in range(3)] + [BUTTON_ENTER, BUTTON_ENTER] +
                # 1
                [BUTTON_PAGE, BUTTON_ENTER] +
                # 0
                [BUTTON_PAGE for _ in range(3)] + [BUTTON_ENTER] +
