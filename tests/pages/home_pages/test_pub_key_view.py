...
def test_silent_payment_scan_key_exports(
    ...
    BTN_SEQUENCE = [
        BUTTON_ENTER,  # Accept the "scan key is private" warning
...
def test_silent_payment_scan_key_warning_declined(
    mocker, m5stickv, tdata, mock_save_file, mock_seed_qr_view
):
    """Declining the privacy warning must export nothing.

    The scan key is private key material; the export screens must not be
    reachable without an explicit confirmation.
    """
    from krux.pages.home_pages.pub_key_view import PubkeyView
    ...
    wallet = Wallet(tdata.SILENT_PAYMENT_KEY)
    ctx = create_ctx(mocker, [BUTTON_PAGE], wallet)  # BUTTON_PAGE == "No"

    assert PubkeyView(ctx).public_key() == MENU_CONTINUE
    mock_save_file.assert_not_called()
    mock_seed_qr_view.assert_not_called()
