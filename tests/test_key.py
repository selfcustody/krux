...
def test_pick_final_word(mocker):
    ...
    assert (
        pick_final_word(
            mock.MagicMock(input=mock.MagicMock(entropy=123456789)),
            TEST_12_WORD_MNEMONIC.split()[:-1],
        )
        == "army"
    )
    assert (
        pick_final_word(
            mock.MagicMock(input=mock.MagicMock(entropy=123456789)),
            TEST_24_WORD_MNEMONIC.split()[:-1],
        )
        == "habit"
    )
    assert (
        pick_final_word(
            mock.MagicMock(input=mock.MagicMock(entropy=987654321)),
            TEST_12_WORD_MNEMONIC.split()[:-1],
        )
        == "situate"
    )
