...
TEST_PARTS_FORMAT_NONE = ['']
...
def test_parser():
    ...
    cases = [
        (FORMAT_NONE, TEST_PARTS_FORMAT_NONE),
        (FORMAT_PMOFN, TEST_PARTS_FORMAT_PMOFN),
        (FORMAT_UR, TEST_PARTS_FORMAT_UR)
    ]
    for case in cases:
        fmt = case[0]
        parts = case[1]
        ...
        if fmt == FORMAT_UR:
            ...
            assert PSBT.from_cbor(res.cbor).data == TEST_DATA_BYTES
        else:
            assert isinstance(res, str)
            assert res == TEST_DATA_B58
...
def test_to_qr_codes(mocker):
    ...
    cases = [
        ...
        (FORMAT_PMOFN, TEST_DATA_B58, TEST_PARTS_FORMAT_PMOFN),
        (FORMAT_UR, TEST_DATA_UR, TEST_PARTS_FORMAT_UR)
    ]
    for case in cases:
        fmt = case[0]
        data = case[1]
