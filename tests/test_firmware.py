...
class MockSuccessInput(mock.MagicMock):
    def wait_for_button(self):
        return BUTTON_ENTER


class MockFailInput(mock.MagicMock):
    def wait_for_button(self):
        return BUTTON_PAGE


def test_find_active_firmware():
    from krux.firmware import find_active_firmware

    for test_sector_case in TEST_SECTOR_CASES:
...
def test_update_boot_config_sector():
    from krux.firmware import update_boot_config_sector

    before_sector = SECTOR_WITH_NO_ACTIVE_FIRMWARE
    for i in range(8):
...
def test_fsize():
    ...
    expected_size = len(open(TEST_FIRMWARE_FILENAME, "rb").read())
    assert fsize(TEST_FIRMWARE_FILENAME) == expected_size


def test_sha256():
    from krux.firmware import sha256

    firmware = open(TEST_FIRMWARE_FILENAME, "rb").read()
    size = len(firmware)
    expected_hash = hashlib.sha256(b"\x00" + size.to_bytes(4, "little") + firmware)
    assert sha256(TEST_FIRMWARE_FILENAME, size) == expected_hash.digest()


def test_write_data_with_header_and_sha_suffix(mocker):
    mocker.patch("krux.firmware.flash", new=mock.MagicMock())
    import krux
    from krux.firmware import write_data
    ...
    data = bytes(SECTOR_WITH_NO_ACTIVE_FIRMWARE)
    size = len(data)
    ...
    krux.firmware.flash.erase.assert_has_calls(
        [
            mock.call(FIRMWARE_SLOT_1, 1024),
            mock.call(FIRMWARE_SLOT_1 + 1024, 1024),
            mock.call(FIRMWARE_SLOT_1 + 2048, 1024),
            mock.call(FIRMWARE_SLOT_1 + 3072, 1024),
            mock.call(FIRMWARE_SLOT_1 + 4096, 1024),
        ]
    )
    krux.firmware.flash.write.assert_has_calls(
        [
            mock.call(FIRMWARE_SLOT_1, header),
            mock.call(FIRMWARE_SLOT_1 + 5, data[: 1024 - 5]),
            mock.call(FIRMWARE_SLOT_1 + 1024, data[1024 - 5 : 1024 - 5 + 1024]),
            mock.call(
...
def test_write_data_with_header_and_no_sha_suffix(mocker):
    mocker.patch("krux.firmware.flash", new=mock.MagicMock())
    import krux
    from krux.firmware import write_data
...
def test_write_data_with_no_header_and_sha_suffix(mocker):
    mocker.patch("krux.firmware.flash", new=mock.MagicMock())
    import krux
    from krux.firmware import write_data
    ...
    krux.firmware.flash.write.assert_has_calls(
        [
            mock.call(FIRMWARE_SLOT_1, data[:1024]),
            mock.call(FIRMWARE_SLOT_1 + 1024, data[1024 : 1024 + 1024]),
            mock.call(FIRMWARE_SLOT_1 + 2048, data[1024 + 1024 : 1024 + 2 * 1024]),
            mock.call(FIRMWARE_SLOT_1 + 3072, data[1024 + 2 * 1024 : 1024 + 3 * 1024]),
            mock.call(
                FIRMWARE_SLOT_1 + 4096,
...
def test_write_data_with_no_header_and_no_sha_suffix(mocker):
    mocker.patch("krux.firmware.flash", new=mock.MagicMock())
    import krux
    from krux.firmware import write_data
...
def test_write_data_with_1_small_read(mocker):
    mocker.patch("krux.firmware.flash", new=mock.MagicMock())
    import krux
    from krux.firmware import write_data
...
def test_write_data_with_1_failed_read(mocker):
    mocker.patch("krux.firmware.flash", new=mock.MagicMock())
    import krux
    from krux.firmware import write_data
...
def test_write_data_with_5_failed_read():
    from krux.firmware import write_data
...
def test_upgrade(mocker):
        ...
        new=get_mock_open(
            {
                ...
                "/sd/firmware-v0.0.0.bin.sig": TEST_FIRMWARE_SIG,
            }
        ),
    )
    mocker.patch(
    ...
    firmware.update_boot_config_sector.assert_called_with(
        bytes(SECTOR_WITH_ACTIVE_FIRMWARE_AT_INDEX_1_SLOT_1),
        1,
        FIRMWARE_SLOT_2,
        len(TEST_FIRMWARE),
    )
    ...
    firmware.write_data.assert_has_calls(
        [
            ...
            mock.call(
                ...
                BACKUP_BOOT_CONFIG_SECTOR_ADDRESS,
                mock.ANY,
                len(bytes(SECTOR_WITH_ACTIVE_FIRMWARE_AT_INDEX_1_SLOT_1)),
                4096,
            ),
            mock.call(
                mock.ANY,
...
def test_upgrade_uses_backup_sector_when_main_sector_is_missing_active_firmware(mocker):
    ...
    mocker.patch(
        ...
        new=mock.MagicMock(
            side_effect=[
                bytes(SECTOR_WITH_NO_ACTIVE_FIRMWARE),
                bytes(SECTOR_WITH_ACTIVE_FIRMWARE_AT_INDEX_1_SLOT_1),
            ]
        ),
    )
    ...
    krux.firmware.flash.read.assert_has_calls(
        [
            mock.call(MAIN_BOOT_CONFIG_SECTOR_ADDRESS, 4096),
            mock.call(BACKUP_BOOT_CONFIG_SECTOR_ADDRESS, 4096),
        ]
    )
    ...
    firmware.write_data.assert_has_calls(
        [
            ...
            mock.call(
                ...
                BACKUP_BOOT_CONFIG_SECTOR_ADDRESS,
                mock.ANY,
                len(bytes(SECTOR_WITH_NO_ACTIVE_FIRMWARE)),
                4096,
            ),
            mock.call(
                mock.ANY,
...
def test_upgrade_uses_slot_1_when_firmware_is_in_slot_2(mocker):
    ...
    firmware.update_boot_config_sector.assert_called_with(
        bytes(SECTOR_WITH_ACTIVE_FIRMWARE_AT_INDEX_1_SLOT_2),
        1,
        FIRMWARE_SLOT_1,
        len(TEST_FIRMWARE),
    )
    ...
    firmware.write_data.assert_has_calls(
        [
            ...
            mock.call(
                ...
                BACKUP_BOOT_CONFIG_SECTOR_ADDRESS,
                mock.ANY,
                len(bytes(SECTOR_WITH_ACTIVE_FIRMWARE_AT_INDEX_1_SLOT_2)),
                4096,
            ),
            mock.call(
                mock.ANY,
...
def test_upgrade_fails_when_sd_card_not_present(mocker):
    mocker.patch("os.listdir", new=mock.MagicMock(side_effect=Exception))
    from krux import firmware

    assert not firmware.upgrade()


def test_upgrade_fails_when_firmware_files_not_present(mocker):
    mocker.patch("os.listdir", new=mock.MagicMock(return_value=["file1"]))
    from krux import firmware

    assert not firmware.upgrade()


def test_upgrade_fails_when_user_declines(mocker):
    mocker.patch(
        "builtins.open",
        new=get_mock_open(
            {
    ...
    mocker.patch("krux.firmware.Display", new=mock.MagicMock())
    mocker.patch("krux.firmware.Input", new=MockFailInput)
    from krux import firmware

    assert not firmware.upgrade()


def test_upgrade_fails_when_firmware_too_big(mocker):
    mocker.patch(
        "builtins.open",
        new=get_mock_open(
            {
    ...
    mocker.patch("krux.firmware.Display", new=mock.MagicMock())
    mocker.patch("krux.firmware.Input", new=MockSuccessInput)
    mocker.patch("krux.firmware.fsize", new=lambda f: MAX_FIRMWARE_SIZE + 1)
    from krux import firmware
...
def test_upgrade_fails_when_sig_file_missing(mocker):
    mocker.patch(
        "builtins.open",
        new=get_mock_open(
            {
...
def test_upgrade_fails_when_both_sectors_missing_active_firmware(mocker):
    ...
    mocker.patch(
        "krux.firmware.flash.read",
        new=mock.MagicMock(return_value=bytes(SECTOR_WITH_NO_ACTIVE_FIRMWARE)),
    )
    from krux import firmware
