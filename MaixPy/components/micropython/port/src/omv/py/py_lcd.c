        ...
        for (int i = 0; i < scale; i++)
        {
            ...
            for (int og_x = 0; og_x < starting_size; og_x++)
            {
                for (int j = 0; j < scale; j++)
                {
                    ...
                    imlib_set_pixel(&arg_img, x, y, code_str[og_yx_index] == '1' ? 0x4208 : DARKGREY);
                }
            }
        }
    }
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(py_lcd_draw_qr_code_obj, 3, 3, py_lcd_draw_qr_code);
